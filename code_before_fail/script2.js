...
// Exercise Logging & Progress Tracker Tab Logic
...
logSetButton.addEventListener('click', () => {
  ...
  if (selectedExercise && weight && reps) {
    ...
    // Update set counter and history
    currentSet++;
    setNumberDisplay.textContent = `Set ${currentSet}`;
...
// Display Workout History in History Tab with Accordion Style
function displayWorkoutHistory() {
  ...
  // Loop through grouped workouts and create an accordion widget for each workout session
  Object.keys(groupedWorkouts).forEach(date => {
    const workoutGroup = groupedWorkouts[date];
    const sessionDiv = document.createElement('div');
    sessionDiv.classList.add('mb-4', 'bg-gray-600', 'rounded-lg');

    // Create a button to toggle the session visibility
    const sessionButton = document.createElement('button');
    sessionButton.classList.add('w-full', 'text-left', 'p-4', 'bg-gray-500', 'text-white', 'font-semibold');
    sessionButton.textContent = `${date} - ${workoutGroup.length} Exercises`;
    
    // Create a div to hold the details of the workout session
    const sessionDetails = document.createElement('div');
    sessionDetails.classList.add('session-details', 'hidden', 'bg-gray-700', 'p-4');
    workoutGroup.forEach(workout => {
      const workoutItem = document.createElement('div');
      workoutItem.classList.add('mb-3');
      workoutItem.innerHTML = `
        <p class="text-lg">${workout.exercise} - Set ${workout.set}</p>
        <p>Weight: ${workout.weight}kg, Reps: ${workout.reps}</p>
        <p>Date: ${workout.date}, Time: ${workout.time}</p>
        <button class="deleteButton bg-red-500 text-white p-2 rounded mt-2" data-date="${workout.date}" data-set="${workout.set}">Delete</button>
      `;
      sessionDetails.appendChild(workoutItem);
    });

    // Add event listener to toggle session details
    sessionButton.addEventListener('click', () => {
      sessionDetails.classList.toggle('hidden');
    });

    sessionDiv.appendChild(sessionButton);
    sessionDiv.appendChild(sessionDetails);
    workoutHistorySection.appendChild(sessionDiv);
  });

  // Add delete functionality to each workout
  const deleteButtons = document.querySelectorAll('.deleteButton');
  deleteButtons.forEach(button => {
    button.addEventListener('click', (event) => {
      const date = event.target.getAttribute('data-date');
      const set = event.target.getAttribute('data-set');
      deleteWorkout(date, set);
    });
  });
}

// Delete Workout from LocalStorage
function deleteWorkout(date, set) {
  let workoutHistory = JSON.parse(localStorage.getItem('workoutHistory')) || [];
  workoutHistory = workoutHistory.filter(workout => !(workout.date === date && workout.set == set));
  localStorage.setItem('workoutHistory', JSON.stringify(workoutHistory));

  // Re-display the workout history after deletion
